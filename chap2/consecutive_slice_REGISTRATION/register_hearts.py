...
output_base = "registration_outputs"
...
# === Identify slice sequences to register
slice_files = sorted([f for f in os.listdir(input_folder) if f.endswith(".h5ad")])

# Group by heart type
groups = {"VSNL1_MUT_A1": [], "VSNL1_MUT_D1": [], "WT_A1": [], "WT_D1": []}
...
for g in groups:
    groups[g].sort(key=lambda x: int(re.search(r"heart_(\d+)", x).group(1)))

# === Register pairwise for each group
for gname, flist in groups.items():
    for i in range(len(flist)-1):
        f1 = flist[i]
        f2 = flist[i+1]
        slice1_path = os.path.join(input_folder, f1)
        slice2_path = os.path.join(input_folder, f2)

        output_dir = os.path.join(output_base, f"{gname}_{f1.replace('.h5ad','')}_to_{f2.replace('.h5ad','')}")
