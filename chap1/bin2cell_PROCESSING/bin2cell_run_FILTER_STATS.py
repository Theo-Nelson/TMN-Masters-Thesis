...
for sample_id, btf_path in samples.items():
    print(f"\n🚀 Running Bin2Cell for {sample_id}...")

    input_dir = f"{sample_id}_square_002um"
    spatial_dir = os.path.join(input_dir, "spatial")
    output_dir = f"{sample_id}_bin2cell_output"
    stardist_dir = os.path.join(output_dir, "stardist")
    ...
    # Print gene/cell counts before filtering
    ...
    # Print gene/cell counts after filtering
    ...
    b2c.grid_image(adata, "n_counts_adjusted", mpp=mpp, sigma=5, save_path=gex_img_path)
    ...
    # Save outputs
    adata.write(os.path.join(output_dir, f"{sample_id}_bins.h5ad"))
    cdata.write(os.path.join(output_dir, f"{sample_id}_cells.h5ad"))
    '''
    print(f"✅ Finished Bin2Cell for {sample_id}. Output saved to: {output_dir}")
